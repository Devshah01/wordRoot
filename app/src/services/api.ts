...
export const api = {
  auth: {
    sendOtp: (body: { email: string; username?: string; isSignUp?: boolean }) =>
      request('/api/auth/send-otp', { method: 'POST', body: JSON.stringify(body) }),
    verifyOtp: (body: { email: string; code: string; username?: string; isSignUp?: boolean }) =>
